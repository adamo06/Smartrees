...
close_date = smeq.closest_image(date0,
                                formatDate=1,
                                pos=tuple([float(item) for item in Pos]))
...
df = smeq.get_meta_data(date_start=str(close_date),
                        date_stop=str(close_date + timedelta(days=1)),
                        pos=tuple([float(item) for item in Pos]))
st.write('Selected date is ', close_date)
smart = smgdf.SmarTrees(ee_image=df.loc[0, 'id'],
                        pos=tuple([float(item) for item in Pos]))
...
m = smart.display_folium_map(min_temp=0,
                             max_temp=40,
                             minNDVI=minNDVI,
                             maxNDVI=maxNDVI)
folium_static(m)
...
if st.button('Show'):
    ...
    Tree_necessity_index = (index + abs(min(index))) / (max(index) -
                                                        min(index)) * 100

    def fill_zeros(data=Tree_necessity_index,
                   size=shapes[10][0] * shapes[10][1]):
